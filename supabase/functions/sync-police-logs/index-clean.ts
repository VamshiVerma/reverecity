...
async function findUnsyncedLogs(supabase, allLogs) {
  const result = await supabase
    .from("police_logs_sync_status")
    .select("sync_date, status")
    .eq("status", "success");

  const syncedLogs = result.data;
  const syncedDates = new Set(syncedLogs ? syncedLogs.map(s => s.sync_date) : []);
...
function parseLogEntries(markdown, logDate, sourceUrl) {
  const entries = [];
  ...
  for (const line of lines) {
    ...
    if (entryMatch) {
      ...
      currentEntry = {
        call_number: entryMatch[1],
        log_date: currentDate.toISOString().split("T")[0],
        time_24h: entryMatch[2],
        timestamp: buildTimestamp(currentDate, entryMatch[2]).toISOString(),
        call_reason: entryMatch[3].trim(),
        action: entryMatch[4].trim(),
  ...
  console.log(`[PARSE] Parsed ${entries.length} entries`);
  return entries;
}

function buildTimestamp(date, time24h) {
...
async function syncDiscoveredLog(supabase, log) {
